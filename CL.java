class CL {
 ...
 /*    Variables Publicas Arreglos    */
   ...
   public static String[] listaocupados = new String[95];
  ...
  /*              Rutinas             */
   ...
   public static void ubicar_palabra(String pf_palabra) { 
     ...
     // Checar por adelantado los espacios que ocupara la palabra
     tempINT = 0;
     do { for (int i = 0; i < largo; i++) {
       ...
       if (Xweight < 0 || Yweight < 0) { 
         for (int j = 0; j < listaocupados.length; j++)  { 
         String add = address_table[Ystart - iYweight][Xstart - iXweight];
         String list = listaocupados[j];
         if (address_table[Ystart - iYweight][Xstart - iXweight] == listaocupados[j]) 
         {System.out.println(current_word + " <" + add + "-" + list + ">"); generarCordenadas();  j = 0; } else {System.out.println(current_word +  " $" + add + "-" + list + "$"); tempINT = 1; } } }
       else{
         for (int j = 0; j < listaocupados.length; j++)  { 
         String add = address_table[Ystart + iYweight][Xstart + iXweight];
         String list = listaocupados[j];
     ...
     for (int i = 0; i < largo; i++) {
       ...
       if (Xweight < 0 || Yweight < 0) { 
         ...
         listaocupados[((current_word-1)*12)+i] = address_table[Ystart - iYweight][Xstart - iXweight];
